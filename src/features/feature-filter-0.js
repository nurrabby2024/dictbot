// Filter handling for DictBot.
const { EventEmitter } = require('node:events');

class FilterHandler extends EventEmitter {
 ...
 if (!payload) throw new Error('empty ' + 'filter' + ' payload');
 return { 'filter': String(payload).slice(0, 200) };
 }
...
module.exports = { FilterHandler, run: (payload, opts) => new FilterHandler().run(payload, opts) };