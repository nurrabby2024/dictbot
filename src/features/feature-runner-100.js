// Runner handling for DictBot.
const { EventEmitter } = require('node:events');

class RunnerHandler extends EventEmitter {
 ...
 if (!payload) throw new Error('empty ' + 'runner' + ' payload');
 return { 'runner': String(payload).slice(0, 200) };
 }
...
module.exports = { RunnerHandler, run: (payload, opts) => new RunnerHandler().run(payload, opts) };